...
load_dotenv(r"C:\Users\thorxme\Desktop\test-my-knowledge-2.o\.env")

# Initialize FastAPI app
app = FastAPI(
    title="Video to Quiz Platform",
    description="A platform that converts YouTube videos into interactive quizzes",
    version="2.0.0"
)
...
async def generate_quiz(request: QuizGenerationRequest, background_tasks: BackgroundTasks):
    ...
    """
    try:
        logger.info(f"Starting quiz generation for video ID: {request.video_id}, difficulty: {request.difficulty}, questions: {request.num_questions}")
...
@app.get("/api/audio/status/{video_id}")
...
@app.get("/api/quiz/status/{video_id}/{difficulty}")
...
@app.post("/api/quiz/start/{video_id}/{difficulty}")
...
@app.get("/api/quiz/results/{video_id}/{difficulty}")
...
@app.get("/api/quiz/question/{video_id}/{difficulty}/{question_index}")
...
@app.delete("/api/quiz/cleanup/{video_id}/{difficulty}")
